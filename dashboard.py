...
def load_sales_data() -> pd.DataFrame:
    ...
    for f in files:
        try:
            # Skip first junk row (e.g. "All the values shown here are in INR.")
        ...
        # Charges = sum of all charge components (as per your formula)
        charge_components = [
            "Restaurant Charge",
            "Packaging Charge [CART - SWIGGY]",
            "Restaurant Packaging Charges",
        ...
        df["MonthKey"] = df["Date"].dt.to_period("M").astype(str)  # e.g. "2025-11"
        df["MonthLabel"] = df["Date"].dt.strftime("%b-%Y")         # e.g. "Nov-2025"
    ...
    # Remove empty or fully-NA frames to avoid FutureWarning
    valid_frames = []
    for fdf in frames:
        if fdf is not None and not fdf.empty:
            if fdf.dropna(how="all").shape[1] > 0:
                valid_frames.append(fdf)
...
def build_summary_and_mis(df: pd.DataFrame):
    ...
    WTD/LWTD/MTD/LMTD are all anchored on yesterday = max(Date in df).
    """
    if df.empty:
    ...
    # Comparison % (WTD vs LWTD, MTD vs LMTD)
    ...
    # Month & WeekNumber will be filled later using filtered df
    summary["Month"] = ""
...
# Month filter (M1: only months in data)
month_options = (
    [
        {"label": ml, "value": mk}
        for mk, ml in (
...
def update_dashboard(
    ...
    sales_df = load_sales_data() if n_clicks else sales_df
    df = sales_df.copy()
