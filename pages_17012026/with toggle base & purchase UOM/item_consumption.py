...
# def round_by_uom(row):
#     uom = str(row.get('Purchase UOM', '')).upper()
#     decimals = UOM_DECIMALS.get(uom, 2)
#     return round(row['Consumed Qty Purchase UOM'], decimals)
...
# ===============================
possible_sales_outlet_cols = [
    'Deployment Name', 'Outlet', 'Outlet Name', 'Store Name'
]

SALES_OUTLET_COL = None
for c in possible_sales_outlet_cols:
    if c in df.columns:
        SALES_OUTLET_COL = c
        break

if SALES_OUTLET_COL is None:
    raise ValueError("❌ No outlet column found in sales data")

print("Using sales outlet column:", SALES_OUTLET_COL)
...
# Year short (25 instead of 2025)
df['YearShort'] = df['Bill Date'].dt.strftime('%y')
df = df[df['Week'].notna()]
df = df[df['YearShort'].notna()]
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            dcc.Dropdown(
                id="f_fy",
                options=[{"label": x, "value": x} for x in sorted(df['FY'].unique())],
            ...
            dcc.Dropdown(
                id="f_item",
                options=[{"label": x, "value": x} for x in sorted(df['Item Name'].dropna().unique())],
            ...
            dcc.Dropdown(
                id="f_ing",
                options=[{"label": x, "value": x} for x in sorted(df['Ingredient Name'].dropna().unique())],
...
def cascade_filters(brand, region, state, city, type_, outlet):
...
def update_tables(
    ...
    if start_date and end_date:
        filtered = filtered[
            (filtered['Bill Date'] >= start_date) &
            (filtered['Bill Date'] <= end_date)
        ]
    else:
        filtered = filtered[
            filtered['Bill Date'].dt.date == YESTERDAY.date()
        ]
    ...
    outlet_tbl = (
        filtered.groupby(
            [SALES_OUTLET_COL, 'Ingredient Name', UOM_COL],
            as_index=False
            )[QTY_COL].sum()
        )
...
def update_ingredient_options(selected_item):
    ...
    # If no item selected → show all ingredients
    if not selected_item:
        return [
            {"label": x, "value": x}
            for x in sorted(df['Ingredient Name'].dropna().unique())
        ]
    ...
    # Filter ingredients linked to selected item(s)
    ing_df = df[df['Item Name'].isin(selected_item)]

    ingredients = sorted(ing_df['Ingredient Name'].dropna().unique())

    return [{"label": x, "value": x} for x in ingredients]
