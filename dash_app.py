# app.py - DSR Dynamic Dashboard with CSV auto-clean, cascading filters, charts & weekly MIS
# Folders:
#   Sales CSVs: C:\Users\ACER\store_dashboard\sales_dashboard\*.csv
#   Store DB : C:\Users\ACER\store_dashboard\stores_db.csv
...
def smart_read_csv(file_path: str) -> pd.DataFrame:
    ...
    for skip in range(0, 5):
        ...
        # If this attempt has more columns, treat as better header guess
    ...
    # If still only 1 column => probably garbage
...
def load_sales_data() -> pd.DataFrame:
    ...
    for f in files:
        ...
        # Ensure key columns (create if missing)
        required_cols = [
            "Region",
            "Outlet Name",
            "Date",
...
# Pass 1 — Normal parse (fast)
        ...
        # Charges = sum of all charge components
        charge_components = [
            "Restaurant Charge",
            "Packaging Charge [CART - SWIGGY]",
            "Restaurant Packaging Charges",
    ...
    # Merge stores DB if available (for City, Area, etc.)
...
def compute_group_kpis(df_period: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns rows with Region, Outlet Name, SourceType, Days, FinalNet, Bills, AOV, ADT, ADS
    SourceType includes Dine In / Delivery / App / Other / Overall.
    Overall is built only from Dine In + Delivery + App.
    """
    if df_period.empty:
        return pd.DataFrame(
            columns=[
    ...
    # Overall metrics from Dine In + Delivery + App only
    ...
    overall = (
        ...
        .agg(
            Days=("Days", "max"),   # assume same dates across channels
...
def period_kpis(df_period: pd.DataFrame, period_name: str) -> pd.DataFrame:
    ...
    # Mix per SourceType relative to Overall ADS
    overall_ads = (
...
def build_weekly_mis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Overall is from Dine In + Delivery + App only.
    """
    if df.empty:
        return pd.DataFrame()
    ...
    for (year, week), df_week in d.groupby(["Year", "WeekNumber"]):
        ...
        g = group.merge(
            overall_ads, on=["Region", "Outlet Name"], how="left"
        )
...
def build_summary_and_mis(df: pd.DataFrame):
    """
    Build summary (period KPIs), MTD MIS, Weekly MIS.
    WTD/LWTD/MTD/LMTD are anchored on yesterday = max(Date in df).
    """
    if df.empty:
    ...
    # Remove empty frames before concat
    valid_period_frames = [
...
# Month filter (only months in data)
month_options = (
    [
        {"label": ml, "value": mk}
        for mk, ml in (
...
# No "Overall" in Source filter
source_options = [
    {"label": s, "value": s}
    for s in ["Dine In", "Delivery", "App", "Other", "Mix"]
]

# Category filter includes "Overall"
category_options = [
...
app.layout = dbc.Container(
    ...
    children=[
        ...
        # Top filters: Region, City, Outlet, Source
        dbc.Row(
            [
                dbc.Col(
                    [
        ...
        # Second row: Category, Month, Week, Day, Refresh
        dbc.Row(
            [
                dbc.Col(
                    [
...
def update_city_outlet_options(region_vals, city_vals, n_clicks):
    """Cascading Region → City → Outlet mapping."""
    global sales_df
    df = sales_df
    if df.empty:
        return [], []

    # Filter by region for city options
    if region_vals:
...
def update_week_options(month_val, n_clicks):
    """Week options mapped with Month."""
    global sales_df
    df = sales_df
    if df.empty:
        return []
...
def update_dashboard(
    ...
    # Build summary & MIS from filtered df
    summary, mis, weekly = build_summary_and_mis(df)

    # ---------------- Summary Table ----------------
    if not summary.empty:
        # Fill Month & WeekNumber based on filtered df ("running month" in context)
        if not df.empty:
            yesterday = df["Date"].max()
        ...
        # Source filter (ignore if "Overall" category selected)
        if source_vals and not (category_vals and "Overall" in category_vals):
            mask = False
            # If Mix selected as source
        ...
        # Category filter including "Overall"
        if category_vals:
    ...
    if not weekly.empty:
        ...
        # Map City → Outlet Names (for weekly)
        if city_vals and "City" in df.columns:
            if isinstance(city_vals, list):
    ...
    # ADS trend: overall Final_Net_Sale per date per outlet
    if df.empty:
    ...
    # AOV vs Source (MTD) and Monthly ADS (MTD vs LMTD)
    if summary.empty:
