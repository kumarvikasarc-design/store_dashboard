...
def clean_common(df, source):
    ...
    df["created_date"] = (
    pd.to_datetime(df["created_date"], errors="coerce", format="mixed", dayfirst=True)
    )
    ...
    df["review_hash"] = hash_string.apply(
    ...
    for file in os.listdir(path):
        if file.endswith(".xlsx"):
            ...
            for sheet in xl.sheet_names:
                ...
                df2 = pd.DataFrame({
                    "created_date": df.get("created at"),
                    "outlet_name": df.get("Please select"),
                    "rating": df[rating_col].replace(rating_map),
                    "comment": df.get("Tell us more"),
                    "customer_name": df.get("Full Name"),
        ...
        if file.endswith(".xlsx") and "Quick Feedback Forms" in xl.sheet_names:
            ...
            qf = pd.DataFrame({
                    "created_date": pd.to_datetime(df_qf["created at"], errors="coerce"),
