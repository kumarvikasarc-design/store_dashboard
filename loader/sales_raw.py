...
for file in os.listdir(INCOMING):
    ...
    try:
        ...
        h = file_hash(src)
        if cur.execute("SELECT 1 FROM dbo.file_import_log WHERE file_hash=?", h).fetchone():
        ...
        out = out[
            out[NUMERIC_CHECK].sum(axis=1) != 0
        ]
        ...
        # Define final order
        FINAL_ORDER = [
            "Building_Type","Area","Region","Outlet_Name","Sale_Date","Tabs",
            ...
            "Total_Tax","Total_Amount","Round_Off","Gross_Amount","Covers"
        ]

        for col in FINAL_ORDER:
            if col not in out.columns:
                out[col] = "-" if col in [
                    "Building_Type","Area","Region","Outlet_Name","Tabs"
                ] else 0

        # Reorder
        out = out[FINAL_ORDER]

        TEXT_COLS = ["Building_Type","Area","Region","Outlet_Name","Tabs"]
        ...
        cur.execute("TRUNCATE TABLE dbo.sales_stage")
        cur.execute(f"""
            BULK INSERT dbo.sales_stage
            FROM '{clean_file}'
            WITH (
                FIRSTROW=2,
                FIELDTERMINATOR=',',
        ...
        SELECT
            ...
            COALESCE(CAST(TRY_CAST(Covers AS DECIMAL(18,2)) AS INT), 0)
        FROM dbo.sales_stage;
        """, file)

        cur.execute("""
            INSERT INTO dbo.file_import_log
            (file_name, file_hash, rows_loaded, status)
            SELECT ?, ?, COUNT(*), 'SUCCESS' FROM dbo.sales_stage
        """, file, h)
