...
# -----------------------------------------------------------
# Load daily sales + merge City & Type from sql
# -----------------------------------------------------------
...
def load_daily_sales() -> pd.DataFrame:
    ...
    # ---- Time keys ----
    df["Year"] = df["Date"].dt.year   # ✅ REQUIRED FIX
...
def compute_kpis(df: pd.DataFrame) -> dict:
    ...
    # billing days (days with at least 1 bill)
    billing_days = (
        df[df["No Of Bills"] > 0]
        [["Outlet Name", "Date"]]
        .drop_duplicates()
    ...
    ads = final_net / billing_days if billing_days else 0.0
    adt = bills / billing_days if billing_days else 0.0
...
def register_callbacks(app):
    ...
    @app.callback(
        ...
        Output("sales_tabs_filter", "options"),
    ...
    def update_sales_dashboard(
        ...
        # clear cache when refresh button clicked
        if ctx == "sales_refresh_button":
            load_daily_sales.cache_clear()
    ...
    # --- No data at all ---
        if df_all.empty:
            return make_empty_result(
                brand_opts=build_options(df_all, "Brand"),
                state_opts=build_options(df_all, "State"),
                region_opts=build_options(df_all, "Region"),
                city_opts=build_options(df_all, "City"),
                type_opts=build_options(df_all, "Type"),
                outlet_opts=build_options(df_all, "Outlet Name"),
                month_opts=build_options(df_all, "MonthKey"),
                week_opts=build_options(df_all, "YearWeek"),
            )
...
# ---------------------------------------------------
        ...
        filter_map = {
            "Brand": to_list(brand_vals),
            "State": to_list(state_vals),
            "Region": to_list(region_vals),
            "City": to_list(city_vals),
            "Type": to_list(type_vals),
            "Outlet Name": to_list(outlet_vals),
            "Tabs": to_list(tabs_vals),
        }
        ...
        if day_val and day_val != "ALL":
            if day_val in df_filtered["DayName"].unique():
        ...
        if search_txt:
            df_filtered = df_filtered[
                df_filtered["Outlet Name"]
                .str.lower()
                .str.contains(str(search_txt).lower(), na=False)
            ]
        # ---------------------------------------------------
        # 🔒 BUILD FILTER OPTIONS (CASCADED – CORRECT)
        # ---------------------------------------------------
        ...
        tabs_options   = build_options(df_filtered, "Tabs")
        ...
        if "Tabs" in df.columns:
            ...
            # flag billing days
            df["BillingDayFlag"] = df["No Of Bills"] > 0
            ...
            tab_group = (
                ...
                .agg(
                    ...
                    Bills=("No Of Bills", "sum"),
                    Billing_Days=("BillingDayFlag", "sum"),
                )
                .reset_index()
            )
            ...
            tab_group["ADS"] = np.where(
                tab_group["Billing_Days"] > 0,
                tab_group["Final_Net_Sale"] / tab_group["Billing_Days"],
                0.0,
            )

            tab_group["ADT"] = np.where(
                tab_group["Billing_Days"] > 0,
                tab_group["Bills"] / tab_group["Billing_Days"],
                0.0,
            )
        ...
        return (
            ...
            tabs_options,            # 18 sales_tabs_filter.options
            # VALUES (for RESET)
            ...
            start_date if ctx != "sales_reset_button" else effective_start,
            end_date if ctx != "sales_reset_button" else effective_end,
