...
ITEM_PURCHASE_UOM_MAP = {
    k[0]: v["base_uom"]
    for k, v in ITEM_CONV_MAP.items()
}
...
def build_master_global_df():
    ...
    # ================= FINANCIAL YEAR (FAST) =================
    fy_start = df["date"].dt.year
    fy_end = fy_start + 1

    df["Financial Year"] = np.where(
        df["date"].dt.month >= 4,
        fy_start.astype(str) + "-" + fy_end.astype(str).str[-2:],
        (fy_start - 1).astype(str) + "-" + fy_start.astype(str).str[-2:]
    )
...
def build_display_uom_map(tx_df, opening_df):
    ...
    entry = tx_df[(tx_df["source"] == "ENTRY") & tx_df["UOM"].notna()]
    for r in entry.itertuples(index=False):
        uom_map.setdefault((r.Warehouse, r.Item), r.UOM)
    ...
    for r in indent.itertuples(index=False):
        uom_map.setdefault((r.Warehouse, r.Item), r.UOM)

    if opening_df is not None and not opening_df.empty:
        for r in opening_df.itertuples(index=False):
            uom_map.setdefault((r.warehouse, r.item_name), r.uom)
...
def force_display_uom(df):
    ...
    keys = list(zip(df[COL_WAREHOUSE], df[COL_ITEM]))
    df[COL_UOM] = [DISPLAY_UOM_MAP.get(k, u) for k,u in zip(keys, df[COL_UOM])]
...
# ======================================================
# FAST qty_out_base calculation (vectorized)
# ======================================================

tx_df["qty_out"] = pd.to_numeric(tx_df["qty_out"], errors="coerce").fillna(0)

tx_df[COL_ITEM] = tx_df[COL_ITEM].astype(str).str.strip()
tx_df[COL_UOM]  = tx_df[COL_UOM].astype(str).str.upper().str.strip()

conv_df = pd.DataFrame([
    (k[0], k[1], v["factor"])
    for k, v in ITEM_CONV_MAP.items()
], columns=[COL_ITEM, COL_UOM, "conv_factor"])
...
tx_df["conv_factor"] = tx_df["conv_factor"].fillna(1)

tx_df["qty_out_base"] = tx_df["qty_out"] * tx_df["conv_factor"]
...
def build_summary_table(df):
    ...
    keys = list(zip(summary[COL_WAREHOUSE], summary[COL_ITEM]))
    summary[COL_UOM] = [DISPLAY_UOM_MAP.get(k) for k in keys]
...
def build_global_ledger(tx_df, opening_df=None):
    ...
    df["Stock_Out"] = [
        convert_indent_to_purchase(i, q, u)
        for i, q, u in zip(df[COL_ITEM], df["qty_out"], df[COL_UOM])
    ]
...
def get_available_stock_map(tx_df):
    ...
    Uses qty_out_base for stock deduction
    """
    ...
    if "qty_in" not in tx_df.columns:
        tx_df["qty_in"] = 0

    tx_df["qty_in"] = pd.to_numeric(tx_df["qty_in"], errors="coerce").fillna(0)
    tx_df["qty_out_base"] = pd.to_numeric(tx_df["qty_out_base"], errors="coerce").fillna(0)
    ...
    return grp["available_stock"].to_dict()
...
if not tx_df.empty:
    if "EXPIRY_FIFO" not in tx_df.get("source","").astype(str).unique():
        tx_df = apply_expiry_deduction(tx_df, EXPIRY_GLOBAL)

expiry_summary = build_expiry_summary(EXPIRY_GLOBAL, tx_df)
...
def build_global_aging(tx_df, as_on_date):
    ...
    grp = (
        ...
        .agg(
            ...
            last_move=(COL_DATE, "max"),
        )
    )

    grp["closing"] = grp["stock_in"] - grp["stock_out"]

    # ================= PURCHASE UOM =================
    def find_purchase_uom(item):
        for (it, u), _ in ITEM_CONV_MAP.items():
            if it == item:
                return u
        return None

    grp["UOM"] = grp[COL_ITEM].map(ITEM_PURCHASE_UOM_MAP)

    grp = grp[grp["UOM"].notna()]

    # ================= AGE =================
    grp["Age Days"] = (as_on - grp["last_move"]).dt.days
    ...
    grp["Aging Bucket"] = pd.cut(
        grp["Age Days"],
        bins=[-1,30,60,10000],
        labels=["0–30","31–60","60+"]
    )

    grp["Last Movement Date"] = grp["last_move"].dt.strftime("%d-%m-%Y")
    ...
    return result.sort_values("Age Days", ascending=False)
...
def register_callbacks(app):
    ...
    def cascade_filters(
        ...
        warehouse, supercat, category, item
    ):
    ...
    def update_aging_table(c, i, ed, bucket):
    ...
    def alerts_combined(_):
        ...
        expiry_df = EXPIRY_GLOBAL
        tx_df = TX_DF_GLOBAL
        
        expiry_df = apply_category_skip(expiry_df, "Category Name")
        tx_df = apply_category_skip(tx_df, COL_CATEGORY)
    ...
    def update_expiry_table(bucket):
    ...
    def export_full_stock(n, sd, ed):
        ...
        out = export_full_stock_excel(tx_df, sd, ed)
    ...
    def update_consumption_tab(warehouse):

        df = TX_DF_GLOBAL

        cons = df[df.get("source","") == "INDENT"].copy()

        cons = apply_category_skip(cons, COL_CATEGORY)
...
if len(scheduler.get_jobs()) == 0:
    ...
    scheduler.add_job(send_negative_stock_mail, "cron", hour=9, minute=5)
    scheduler.add_job(send_available_stock_snapshot, "cron", day="last", hour=9, minute=10)
