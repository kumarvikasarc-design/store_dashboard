...
# 🔥 CORRECT KEY MAPPING FROM SQL
EMAIL_FROM = EMAIL_CONFIG.get("EMAIL_SENDER", "").strip()
EMAIL_PASS = EMAIL_CONFIG.get("GMAIL_APP_PASSWORD", "").replace(" ", "").strip()
...
def send_email_html(subject, html_body, images):
    ...
    # ----------- parse emails safely -----------
    to_emails  = parse_emails(EMAIL_TO)  or []
    cc_emails  = parse_emails(EMAIL_CC)  or []
    bcc_emails = parse_emails(EMAIL_BCC) or []
    ...
    # ----------- stop if no TO -----------
    if not to_emails:
        print("⚠️ No recipients found. Mail skipped.")
        return
    ...
    print("📨 MAIL SENDING TO:", recipients)
...
def send_ops_email_html(subject, html_body, images, ops_excel_part):

    # ---------------- GET EMAIL LIST ----------------
    ops_to  = OPS_TO_EMAILS or []
    ops_cc  = OPS_CC_EMAILS or []
    ops_bcc = OPS_BCC_EMAILS or []
    
    # ---- Safety: ensure always list ----
    if isinstance(ops_to, str):
        ops_to = [ops_to]

    if isinstance(ops_cc, str):
        ops_cc = [ops_cc]

    if isinstance(ops_bcc, str):
        ops_bcc = [ops_bcc]
    ...
    print("📨 OPS MAIL SENDING TO:", recipients)
    ...
    try:
        ...
        print("OPS_TO:", ops_to)
        print("OPS_CC:", ops_cc)
        print("OPS_BCC:", ops_bcc)
        print("FINAL:", recipients)

        print("✅ OPS EMAIL SENT SUCCESS")
    except Exception as e:
...
def send_store_wise_emails(
    ...
    for outlet_norm, g in merged.groupby("Outlet_norm"):
        ...
        for e in [store_email, manager_email]:
            if pd.notna(e):
                email_str = str(e).strip()
                if email_str:
                    to_emails.append(email_str)
        ...
        try:
            ...
            print(f"✅ Store email sent → {store_name}")

        except Exception as e:
            print(f"❌ Store email failed → {store_name}: {e}")           
#=========================================
...
def register_email_callback(app):
    ...
    def send_report(
        ...
        df_qf = df_all[df_all["Source"] == "Feedback Form"]
