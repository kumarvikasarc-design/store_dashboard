...
def fix_relative_imports_in_text(text, fname):
    ...
    # Additional fixes: "from X import " -> "from X import" (if module name follows)
    # Handle: from something import   --> from something import  (rare)
    ...
    # If we see "from utils..." inside pages or root, convert to "from utils..."
...
if os.path.exists(main_app):
    with open(main_app, "r", encoding="utf-8") as fh:
        mtxt = fh.read()
    # remove any "from utils" or "from utils.auth" leftover
