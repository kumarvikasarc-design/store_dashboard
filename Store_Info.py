...
def generate_table_columns():
    return [
        ...
        {"name": "Area Manager", "id": "Area Manager", "presentation": "dropdown", "editable": False},
        # --- CHANGE 1: Update Live Date column for DD-MM-YYYY display ---
        {
                "name": "Opening Date",
                "id": "Opening Date",
                "type": "text",
    ...
    df["Opening Date"] = pd.to_datetime(df["Opening Date"], errors='coerce').dt.strftime("%d-%m-%Y")

def add_edit_button_to_data(df):
    df = df.copy()
    # The markdown link is needed for the active_cell trigger to work reliably
    if 'edit' not in df.columns or df['edit'].iloc[0] != '[Edit](edit)':
        df['edit'] = '[Edit](edit)' 
    return df
...
app.layout = html.Div([
    ...
    dash_table.DataTable(
        ...
        # Ensure markdown is enabled for the edit link
    ...
    html.Div(id="modal", style=MODAL_STYLE, children=[
        html.Div(style=MODAL_CONTENT_STYLE, children=[
            ...
            html.Label("Opening Date *"),
            # --- CHANGE: Replaced dcc.Input with dcc.DatePickerSingle ---
            dcc.DatePickerSingle(
                id='input-date',
                display_format='DD-MM-YYYY', # User-facing format
                date=None, # Initial date
...
@app.callback(
    ...
    Output("input-date", "date"), # --- CHANGE: Outputting to 'date' property ---
    ...
    State("input-date", "date"), # --- CHANGE: Reading from 'date' property ---
...
def unified_callback(
    ...
    input_date = no_update # --- CHANGE: Initialized as no_update for DatePicker ---
    ...
    if trig == "reset-btn" and reset_click:
        ...
        input_type = ""
        input_manager = ""
        input_date = None # --- CHANGE: Reset to None for DatePicker ---
        input_status = None
        edit_id = None
    ...
    # --- 3. Handle Modal Open/Close (Add/Cancel/Edit) ---
        ...
        input_outlet, input_region, input_city_value_out, input_type, input_manager, input_status = "", None, None, "", "", None
        input_date = None # --- CHANGE: Reset to None for DatePicker ---
        input_city_options = []
        edit_id = None
        ...
        modal_style = {**MODAL_STYLE, "display": "none"}
        modal_msg = ""
        # Flow continues to filtering step
    ...
    # Handle Edit Button Click
        ...
        if active_cell and active_cell["column_id"] == "edit":
            ...
            if row_idx < len(table_data): 
                ...
                modal_style = {**MODAL_STYLE, "display": "block"} # <-- This opens the modal
                ...
                # input_date must be in DD-MM-YYYY format for DatePicker
                input_date = row.get("Opening Date", None) 
        ...
        # If the trigger was 'table' but not an edit click, flow continues

    # --- 4. Handle City Dropdown Update in Modal (input-region change) ---
    elif trig == "input-region":
        ...
        input_date = input_date_val # DatePicker State value
    ...
    # --- 5. Handle Save Button (Add/Update) ---
        ...
        # input_date_val is now the DD-MM-YYYY string from DatePicker
        ...
        if not all(required_fields):
            ...
            # Date validation is not strictly needed here as DatePicker ensures format,
            # but we can keep it simple to ensure the value isn't empty if required.
            ...
            new_row_data = {
                ...
                # input_date_val is already DD-MM-YYYY format from DatePicker
                "Opening Date": input_date_val, 
    ...
    # --- 6. Handle Upload Excel File ---
        ...
        if upload_contents:
            ...
            try:
                ...
                required_cols = set(["Store Id", "Outlet Name", "Region", "City", "Type", "Area Manager", "Opening Date", "Status"])
    ...
    # --- 7. Filtering the table data based on filters/search (ALWAYS RUN at the end) ---
    ...
    # Determine if the modal should be forced open (Add, Edit, or Save failure)
    is_modal_open = (
        trig == "add-btn" or 
        (trig == "table" and active_cell and active_cell["column_id"] == "edit") or
        (trig == "save-btn" and modal_msg.startswith("❌"))
    )

    # If modal is not intentionally open, ensure inputs are reset for the final return
    if not is_modal_open and trig not in ["input-region"]: # input-region has its own return
        # Reset modal inputs
        input_outlet = ""
        input_region = None
        input_city_options = []
        ...
        input_type = ""
        input_manager = ""
        input_date = None # --- CHANGE: Reset to None ---
        input_status = None
        edit_id = None
...
def download_template(n):
    def generate_excel_template(io):
        df = pd.DataFrame({
            ...
            "Opening Date": ["29-01-2025", "04-06-2025"],
...
def download_xlsx(n):
    ...
    def to_excel(io):
        df.to_excel(io, index=False, sheet_name="Stores", engine='openpyxl')
        io.seek(0)
    return dcc.send_bytes(to_excel, f"StoreInfo_{datetime.now().strftime('%d%m%y')}.xlsx")
...
def download_pdf(n):
    ...
    def to_pdf(io):
        df_to_pdf(io, df)
    return dcc.send_bytes(to_pdf, f"StoreInfo_{datetime.now().strftime('%d%m%y')}.pdf")
