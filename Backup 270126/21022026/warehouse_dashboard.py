...
# =========================================
# LOAD ITEM MASTER FROM SQL (ENTERPRISE)
# =========================================
# ======================================================
...
def load_item_master():
    try:
        ...
        print("✅ Item master loaded:", len(df))
        return df
...
def load_opening_stock_sql():
    try:
        ...
        if df.empty:
            print("⚠️ No opening stock found")
            return pd.DataFrame()

        df.columns = df.columns.str.strip()

        df.rename(columns={
            "warehouse": COL_WAREHOUSE,
            "item_name": COL_ITEM,
            "uom": COL_UOM,
        ...
        df["date"] = pd.to_datetime(df["date"], errors="coerce")
        df["opening_qty"] = pd.to_numeric(df["opening_qty"], errors="coerce").fillna(0)
...
if missing:
    raise Exception(
        f"❌ Item master missing required columns.\n"
        f"Found columns: {ITEM_MASTER.columns.tolist()}"
    )
...
def register_callbacks(app):
    ...
    def cascade_filters(sc, cat, item, wh, _):
        ...
        df = TX_DF_GLOBAL.copy()   # ⚡ fast no SQL reload
    ...
    def update_order_planning_table(_):
        ...
        # 🔥 BUILD ORDER PLAN
        plan_df = build_order_planning(df)
