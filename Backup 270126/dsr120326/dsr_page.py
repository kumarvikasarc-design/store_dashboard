...
def load_dsr_from_sql():
    ...
    SELECT
        ...
        (
            ...
            ISNULL(s.staff_welfare,0) +
            ISNULL(s.staff_welfare_charge_5,0) +
...
# Load data
@lru_cache(maxsize=1)
def _load_sales_data_cached() -> pd.DataFrame:
    return load_dsr_from_sql()

def load_sales_data() -> pd.DataFrame:
    return _load_sales_data_cached().copy()
...
def clean_and_enrich_sales(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d["ISOYear"] = iso.year.astype("Int64")
    d["WeekNumber"] = iso.week.astype("Int64")

    d["YearWeek"] = (
        d["ISOYear"].astype(str)
        + "-W"
        + d["WeekNumber"].astype(str).str.zfill(2)
    )
...
def build_weekly_mis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    d = d.dropna(subset=["ISOYear", "WeekNumber"])


    weekly_list = []

    # ----------------------------- GROUP BY (Year, WeekNumber) -----------------------------
    for (year, week), df_week in d.groupby(["ISOYear", "WeekNumber"]):
...
def get_layout():
    ...
    if not sales_df.empty and "Date" in sales_df.columns:
        ...
        d["FY"] = np.where(
            d["Date"].dt.month >= 4,
            d["Date"].dt.year.astype(str) + "-" + (d["Date"].dt.year + 1).astype(str),
            (d["Date"].dt.year - 1).astype(str) + "-" + d["Date"].dt.year.astype(str)
        )

        fy_options = [
            {"label": f"FY {fy}", "value": fy}
            for fy in sorted(d["FY"].dropna().unique())
        ]
    latest_fy = fy_options[-1]["value"] if fy_options else None
    ...
    return html.Div(
        ...
        # ✅ REQUIRED STORE (FIXES CALLBACK FAILURES)
            dcc.Store(
                id="filters_dsr",
                storage_type="memory"
            ),            
...
# -------------------- FILTER ROW 1 --------------------
                dbc.Row(
                    [
                        dbc.Col(dcc.Dropdown(id="brand_filter_dsr", multi=False, placeholder="Brand"),
                                lg=2, md=4, sm=6, xs=12),

                        dbc.Col(dcc.Dropdown(id="state_filter_dsr", multi=False, placeholder="State"),
                                lg=2, md=4, sm=6, xs=12),

                        dbc.Col(dcc.Dropdown(id="region_filter_dsr", multi=False, placeholder="Region"),
                                lg=2, md=4, sm=6, xs=12),

                        dbc.Col(dcc.Dropdown(id="city_filter_dsr", multi=False, placeholder="City"),
                                lg=2, md=4, sm=6, xs=12),

                        dbc.Col(dcc.Dropdown(id="type_filter_dsr", multi=False, placeholder="Type"),
                                lg=2, md=4, sm=6, xs=12),

                        dbc.Col(dcc.Dropdown(id="outlet_filter_dsr", multi=False, placeholder="Outlet"),
                                lg=2, md=4, sm=6, xs=12),
                    ],
                    className="g-2 mb-2",
                ),
                ...
                dbc.Row(
                    [
                        dbc.Col(dcc.Dropdown(id="source_filter_dsr", options=source_options, multi=False, placeholder="Source"),
                                lg=2, md=4, sm=6, xs=12),

                        dbc.Col(dcc.Dropdown(id="category_filter_dsr", options=category_options, multi=False, placeholder="Category"),
                                lg=2, md=4, sm=6, xs=12),
                        # 🔥 Financial Year filter
                        dbc.Col(
                            dcc.Dropdown(
                                id="financial_year_filter_dsr",
                                options=fy_options,
                                value=[latest_fy] if latest_fy else None,   # ✅ HERE
                                multi=False,
                        ...
                        dbc.Col(dcc.Dropdown(id="month_filter_dsr", options=month_options, placeholder="Month"),
                                lg=2, md=4, sm=6, xs=12),

                        dbc.Col(dcc.Dropdown(id="week_filter_dsr", multi=False, placeholder="Week"),
                                lg=2, md=4, sm=6, xs=12),

                        dbc.Col(dcc.Dropdown(id="day_filter_dsr", options=day_options, value="ALL", placeholder="Day"),
                                lg=2, md=4, sm=6, xs=12),
                    ],
                    className="g-2 mb-2",
                ),

                # -------------------- SEARCH + ACTIONS (SINGLE LINE) --------------------
                dbc.Row(
                    [
                        dbc.Col(
                            [
                                html.Label("Summary search (Outlet Name)"),
                                dcc.Input(
                                    id="search_text_dsr",
                                    type="text",
                        ...
                        dbc.Col(
                            dbc.Button(
                                "Refresh",
                                id="refresh_button_dsr",
                                color="primary",
                                className="w-100 filter-refresh-btn",
                            ),
                        ...
                        dbc.Col(
                            dbc.Button(
                                "Reset Filters",
                                id="reset_filters_btn_dsr",
                                color="secondary",
                                className="w-100 filter-reset_btn",
                            ),
                            lg=3, md=3, sm=6, xs=12,
                            className="d-flex align-items-end",
                        ),
            ...
            # Tabs
            dcc.Tabs(id="tabs_dsr", value="tab-summary", children=[
...
def weeks_from_df(df: pd.DataFrame, month_key: str = None):
    ...
    if dfc.empty:
        return []
    weeks = dfc[["YearWeek", "ISOYear", "WeekNumber"]].drop_duplicates().sort_values(["ISOYear", "WeekNumber"])
    return [{
        "label": f"Wk {int(row.WeekNumber)} - {int(row.ISOYear)}",
        "value": row.YearWeek
...
def register_callbacks(app):
    ...
    @app.callback(
        Output("filters_dsr", "data"),
        Input("brand_filter_dsr", "value"),
        Input("region_filter_dsr", "value"),
        Input("state_filter_dsr", "value"),
        Input("city_filter_dsr", "value"),
        Input("type_filter_dsr", "value"),
        Input("outlet_filter_dsr", "value"),
        Input("source_filter_dsr", "value"),
        Input("category_filter_dsr", "value"),
        Input("financial_year_filter_dsr", "value"),
        Input("month_filter_dsr", "value"),
        Input("week_filter_dsr", "value"),
        Input("day_filter_dsr", "value"),
        Input("search_text_dsr", "value"),
        Input("refresh_button_dsr", "n_clicks"),
        Input("reset_filters_btn_dsr", "n_clicks"),
        prevent_initial_call=True
    )
    def store_dsr_filters(
        brand, region, state, city, type_, outlet,
        source, category, financial_year, month, week, day, search,
        refresh_clicks, reset_clicks
    ):
        ctx = callback_context

        if ctx.triggered and ctx.triggered[0]["prop_id"].startswith("reset_filters_btn_dsr"):
            return {}   # 🔥 CLEAR STORE COMPLETELY
    ...
    @app.callback(
        Output("brand_filter_dsr", "value"),
        Output("state_filter_dsr", "value"),
        Output("region_filter_dsr", "value"),
        Output("city_filter_dsr", "value"),
        Output("type_filter_dsr", "value"),
        Output("outlet_filter_dsr", "value"),
        Output("source_filter_dsr", "value"),
        Output("category_filter_dsr", "value"),
        Output("financial_year_filter_dsr", "value"),
        Output("week_filter_dsr", "value"),
        Output("day_filter_dsr", "value"),
        Output("search_text_dsr", "value"),
        Input("reset_filters_btn_dsr", "n_clicks"),
        prevent_initial_call=True,
    )
    ...
    #     Input("financial_year_filter", "value"),
    #     Input("reset_filters_btn", "n_clicks"),
    #     Input("filters-dsr", "data"),
    #     prevent_initial_call=True,
    # )
    # def control_month_value(fy_value, reset_clicks, filters):
    ...
    #     # ---------- CASE 1: Reset clicked ----------
    #     if trigger.startswith("reset_filters_btn"):
    #         if not fy_value:
    #             return None

    #     # ---------- CASE 2: FY changed ----------
    #     if trigger.startswith("financial_year_filter"):
    #         if not fy_value:
    #             return None

    #     # ---------- CASE 3: Month already selected ----------
    ...
    #     fy_vals = fy_value if isinstance(fy_value, list) else [fy_value]

    #     df["FY"] = np.where(
    #         df["Date"].dt.month >= 4,
    #         df["Date"].dt.year.astype(str) + "-" + (df["Date"].dt.year + 1).astype(str),
    #         (df["Date"].dt.year - 1).astype(str) + "-" + df["Date"].dt.year.astype(str),
    #     )

    #     df_fy = df[df["FY"].isin(fy_vals)]
    #     if df_fy.empty:
    #         return None
    ...
    # CROSS-FILTER callback (State, Region, City, Type, Outlet)
    @app.callback(
        Output("brand_filter_dsr", "options"),
        Output("region_filter_dsr", "options"),
        Output("state_filter_dsr", "options"),
        Output("city_filter_dsr", "options"),
        Output("type_filter_dsr", "options"),
        Output("outlet_filter_dsr", "options"),
        Input("filters_dsr", "data"),
    )
    def cross_filter_dsr(filters):

        df = _get_prepared_sales()
        if df.empty or not filters:
            return (
                [{"label": b, "value": b} for b in sorted(df["Brand"].dropna().unique())] if "Brand" in df.columns else [],
                [{"label": r, "value": r} for r in sorted(df["Region"].dropna().unique())],
                [{"label": s, "value": s} for s in sorted(df["State"].dropna().unique())] if "State" in df.columns else [],
                [{"label": c, "value": c} for c in sorted(df["City"].dropna().unique())] if "City" in df.columns else [],
                [{"label": t, "value": t} for t in sorted(df["Type"].dropna().unique())] if "Type" in df.columns else [],
                [{"label": o, "value": o} for o in sorted(df["Outlet Name"].dropna().unique())],
            )

        d = df.copy()

        # Apply filters from STORE (NOT UI)
        if filters.get("brand"):
            d = d[d["Brand"].isin(as_list(filters.get("brand")))]
        if filters.get("region"):
            d = d[d["Region"].isin(as_list(filters.get("region")))]
        if filters.get("state"):
            d = d[d["State"].isin(as_list(filters.get("state")))]
        if filters.get("city"):
            d = d[d["City"].isin(as_list(filters.get("city")))]
        if filters.get("type"):
            d = d[d["Type"].isin(as_list(filters.get("type")))]
        if filters.get("outlet"):
            d = d[d["Outlet Name"].isin(as_list(filters.get("outlet")))]
    ...
    # MONTH → WEEK (options)
    @app.callback(
        Output("week_filter_dsr", "options"),
        Input("filters_dsr", "data"),
    )
    def update_week_options(filters):

        if not filters:
            return []

        df = _get_prepared_sales()
        return weeks_from_df(df, filters.get("month"))

    # WEEK → MONTH (reverse mapping)
    @app.callback(
        Output("month_filter_dsr", "options"),
        Input("filters_dsr", "data"),
    )
    def update_month_options_from_week(filters):

        if not filters:
            return []

        df = _get_prepared_sales()
        return months_from_weeks(df, filters.get("week"))
    ...
    @app.callback(
        Output("tabs_content_dsr", "children"),
        Input("tabs_dsr", "value"),
        Input("filters_dsr", "data"),
    )
    def render_tab_content(tab_value, filters):
        ...
        if not filters:
            return html.Div(
                "Click on Refresh Button for Running Month Data",
        ...
        state_vals    = filters.get("state")
        region_vals   = filters.get("region")
        city_vals     = filters.get("city")
        type_vals     = filters.get("type")
        outlet_vals   = filters.get("outlet")
        source_vals   = filters.get("source")
        category_vals = filters.get("category")
        month_val     = filters.get("month")
        week_vals     = filters.get("week")
        day_val       = filters.get("day")
        search_txt    = filters.get("search")
    ...
    # --------------------
        if brand_vals and "Brand" in df.columns:
            df = df[df["Brand"].isin(brand_vals if isinstance(brand_vals, list) else [brand_vals])]

        if state_vals and "State" in df.columns:
            df = df[df["State"].isin(state_vals if isinstance(state_vals, list) else [state_vals])]

        if region_vals and "Region" in df.columns:
            df = df[df["Region"].isin(region_vals if isinstance(region_vals, list) else [region_vals])]

        if city_vals and "City" in df.columns:
            df = df[df["City"].isin(city_vals if isinstance(city_vals, list) else [city_vals])]

        if type_vals and "Type" in df.columns:
            df = df[df["Type"].isin(type_vals if isinstance(type_vals, list) else [type_vals])]

        if outlet_vals:
            df = df[df["Outlet Name"].isin(outlet_vals if isinstance(outlet_vals, list) else [outlet_vals])]

        fy_vals = filters.get("financial_year")

        if fy_vals:
            fy_vals = fy_vals if isinstance(fy_vals, list) else [fy_vals]
            ...
            df = df[df["FY"].isin(fy_vals)]

        if month_val:
            df = df[df["MonthKey"] == month_val]

        if week_vals:
            week_vals = week_vals if isinstance(week_vals, list) else [week_vals]
        ...
        if search_txt:
            s = str(search_txt).strip().lower()
            if s:
                df = df[df["Outlet Name"].str.lower().str.contains(s, na=False)]

        # --------------------
        # Build outputs (unchanged logic below)
        # --------------------
        ...
        if not weekly.empty:
            ...
            # Mark current running week
            max_date = df["Date"].max()
            iso = max_date.isocalendar()

            weekly["_is_running"] = (
                (weekly["ISOYear"] == int(iso.year)) &
                (weekly["WeekNumber"] == int(iso.week))
            )

            weekly = weekly.sort_values(
                ["_is_running", "ISOYear", "WeekNumber"],
                ascending=[False, False, False]
            ).drop(columns=["_is_running"], errors="ignore")
            

            # Convert Year/WeekNumber to numeric for safe processing
            weekly["ISOYear_num"] = pd.to_numeric(weekly["ISOYear"], errors="coerce")
            ...
            def compute_week_meta(row):
                year_val = int(row["ISOYear"])
                week_val = int(row["WeekNumber"])

                dsub = df[
                    (df["ISOYear"] == year_val) &
                    (df["WeekNumber"] == week_val)
                ]
                ...
                week_range = f"{start:%d %b} - {end:%d %b}"

                return month, week_range
            ...
            weekly["ISOYear"] = weekly["ISOYear"].apply(
            ...
            # Reorder columns
            desired = ["ISOYear", "Month", "WeekNumber", "WeekRange"]
