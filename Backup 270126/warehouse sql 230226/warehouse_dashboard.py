...
def reload_all_data():
    ...
    try:
        ...
        # qty_out_base (for consumption engine)
        df["qty_out_base"] = df["qty_out"]

        # wastage_amount (for wastage module)
        if "wastage_amount" not in df.columns:
            df["wastage_amount"] = 0

        # ================= SAVE GLOBAL =================
        tx_df = df.copy()
        TX_DF_GLOBAL = df.copy()
...
def build_master_global_df():
    ...
    print("⚡ Building master global DF")
    ...
    print("✅ Master global DF ready")
    print("Columns:", df.columns.tolist())
    print(TX_DF_GLOBAL[["Outlet","Brand","State","Region","City"]].head(20))
    print("Months:", sorted(TX_DF_GLOBAL["Month"].dropna().unique()))
    
tx_df = reload_all_data()

build_master_global_df()   # merges store master + month + FY

# THEN APPLY ITEM MASTER MAPS
TX_DF_GLOBAL["Super Category"] = TX_DF_GLOBAL["Item"].map(ITEM_SUPER_CAT_MAP)
...
# ======================================================
def dropdown_options(series, sort_series=None):
    ...
    if sort_series is not None:
        ...
        sort_series = pd.Series(sort_series)
        temp["sort"] = sort_series
...
def register_callbacks(app):
    ...
    def cascade_filters(
        ...
        # 🚫 Hide warehouse / central store from outlet dropdown
        blocked_outlets = [
            "central store",
            "warehouse",
            "warehouse ncr",
        ...
        outlet_series = df.get("Outlet", pd.Series()).astype(str).str.strip()

        outlet_series = outlet_series[
            ~outlet_series.str.lower().isin(blocked_outlets)
        ]

        outlet_opt = dropdown_options(outlet_series)

        fy_opt      = dropdown_options(df.get("Financial Year", pd.Series()))
        month_opt   = dropdown_options(df.get("Month", pd.Series()),
                                    df.get("Month_Sort", pd.Series()))
