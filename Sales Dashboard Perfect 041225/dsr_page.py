# pages/dsr_page.py
# DSR Dynamic Dashboard page module (for main_app.py)
# - Auto raw data from: C:\Users\ACER\store_dashboard\sales_dashboard\*.csv
# - Month-wise KPI cards (ADS, AOV, ADT, MIX) above summary table
# - Compatible with main_app.py (uses get_layout() + register_callbacks(app))
...
def smart_read_csv(file_path: str) -> pd.DataFrame:
    ...
    - Picks the attempt with the most columns
    ...
    if best_df is None or best_df.shape[1] <= 1:
        print(f"Skipping unreadable or invalid CSV: {file_path}")
        return pd.DataFrame()
...
def load_sales_data() -> pd.DataFrame:
    """
    Load all sales CSVs from SALES_FOLDER, auto-clean and compute:
    - Net Sale = Sale - Discount
    - Charges = sum of charge components
    - Final_Net_Sale = Net Sale + Charges
    - Time keys: Year, WeekNumber, YearWeek, MonthKey, MonthLabel, DayName, DayCode
    """
    ...
    for f in files:
        ...
        for col in required_cols:
            if col not in df.columns:
               ...
               # ---------------------------
# AUTO-DETECT DATE FORMAT (Hybrid D) - Clean version (no warnings)
# ---------------------------

        raw_date = df["Date"].astype(str)

# Pass 1 — Normal parse (fast)
        df["Date"] = pd.to_datetime(raw_date, errors="coerce")

        # Second pass: retry failed parse with dayfirst=True
        mask_failed = df["Date"].isna()
        if mask_failed.any():
            df.loc[mask_failed, "Date"] = pd.to_datetime(
                df.loc[mask_failed, "Date"].astype(str),
                infer_datetime_format=True,
                dayfirst=True,
                errors="coerce",
            )

        # Third pass: Excel-like serial numbers
        mask_failed2 = df["Date"].isna()
        if mask_failed2.any():
            try:
                df.loc[mask_failed2, "Date"] = pd.to_datetime(
                    df.loc[mask_failed2, "Date"].astype(float),
                    unit="d",
                    origin="1899-12-30",
                    errors="coerce",
                )
        ...
        # Drop rows with still-invalid dates
        ...
        df["MonthKey"] = df["Date"].dt.to_period("M").astype(str)   # 2025-11
        df["MonthLabel"] = df["Date"].dt.strftime("%b-%Y")          # Nov-2025
    ...
    # Merge stores DB for City / Type etc.
...
def compute_group_kpis(df_period: pd.DataFrame) -> pd.DataFrame:
    """
    Aggregates to:
    Region, Outlet Name, SourceType, Days, FinalNet, Bills, AOV, ADT, ADS
    SourceType includes Dine In / Delivery / App / Other / Overall
    Overall is built from Dine In + Delivery + App only.
    """
    if df_period.empty:
        return pd.DataFrame(
            columns=[
...
def build_weekly_mis(df: pd.DataFrame) -> pd.DataFrame:
    """
    Weekly MIS: Year, WeekNumber, Region, Outlet Name, Source, AOV, ADT, ADS, Mix
    """
    if df.empty:
        return pd.DataFrame()
...
def build_summary_and_mis(df: pd.DataFrame):
    """
    Build:
      - Summary (WTD, LWTD, MTD, LMTD, Last4Weeks, Yesterday, LWSD, L4WSD)
      - MTD MIS (Region x Source)
      - Weekly MIS
    All anchored on 'yesterday' = max(Date) within df.
    """
    if df.empty:
...
# -------------------------------------------------
# KPI CARD BUILDER (MONTH-WISE, ALL OUTLET / FILTERED)
# -------------------------------------------------
def _format_currency(v: float) -> str:
    return f"₹ {v:,.0f}"


def _format_number(v: float, decimals: int = 1) -> str:
    return f"{v:,.{decimals}f}"


def build_kpi_cards(region_vals, city_vals, outlet_vals, month_val):
    """
    Month-wise ADS, AOV, ADT, MIX (4 cards in one row).

    - Uses global sales_df as base
    - Applies Region / City / Outlet filters
    - Month:
        * If month_val selected → use that MonthKey
        * Else → auto-pick running month (month of max Date in filtered data)
    - MIX = (ADS_filtered / ADS_all_outlets_same_month) * 100
    """
    global sales_df
    df = sales_df.copy()
    if df.empty:
        return html.Div("No data available for KPI", style={"padding": "4px", "fontSize": "13px"})

    # Apply Region / City / Outlet filters to KPI base
    if region_vals:
    ...
    if df.empty:
        return html.Div("No data for selected filters", style={"padding": "4px", "fontSize": "13px"})

    # Month scope
    if month_val:
        df = df[df["MonthKey"] == month_val]
        if df.empty:
            return html.Div("No data for selected month", style={"padding": "4px", "fontSize": "13px"})
        target_month_key = month_val
    else:
        latest_date = df["Date"].max()
        target_month_key = latest_date.to_period("M").strftime("%Y-%m")
        df = df[df["MonthKey"] == target_month_key]
        if df.empty:
            return html.Div("No data for running month", style={"padding": "4px", "fontSize": "13px"})

    # Selected scope KPIs
    days = df["Date"].dt.normalize().nunique()
    total_net = df["Final_Net_Sale"].sum()
    total_bills = df["No Of Bills"].sum()

    ADS = total_net / days if days > 0 else 0.0
    AOV = total_net / total_bills if total_bills > 0 else 0.0
    ADT = total_bills / days if days > 0 else 0.0

    # MIX vs all outlets same month (no region/city/outlet filter)
    df_all = sales_df[sales_df["MonthKey"] == target_month_key]
    if df_all.empty:
        MIX = 0.0
    else:
        days_all = df_all["Date"].dt.normalize().nunique()
        total_net_all = df_all["Final_Net_Sale"].sum()
        ADS_all = total_net_all / days_all if days_all > 0 else 0.0
        MIX = (ADS / ADS_all * 100.0) if ADS_all > 0 else 0.0

    subtitle = f"Month: {df.iloc[0]['MonthLabel']}" if "MonthLabel" in df.columns else ""
    ...
    label_style = {"fontSize": "12px", "color": "#6B7280", "marginBottom": "4px"}
    value_style = {"fontSize": "20px", "fontWeight": "700", "color": "#111827"}
    sub_style = {"fontSize": "11px", "color": "#9CA3AF", "marginTop": "2px"}
        ...
        [
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(
                        [
                            html.Div("ADS (₹)", style=label_style),
                            html.Div(_format_currency(ADS), style=value_style),
                            html.Div(subtitle, style=sub_style),
                        ]
                    ),
                    style=card_style,
                ),
            ...
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(
                        [
                            html.Div("AOV (₹)", style=label_style),
                            html.Div(_format_currency(AOV), style=value_style),
                            html.Div(subtitle, style=sub_style),
                        ]
                    ),
                    style=card_style,
            ...
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(
                        [
                            html.Div("ADT (Bills / Day)", style=label_style),
                            html.Div(_format_number(ADT, 1), style=value_style),
                            html.Div(subtitle, style=sub_style),
                        ]
                    ),
                    style=card_style,
            ...
            dbc.Col(
                dbc.Card(
                    dbc.CardBody(
                        [
                            html.Div("MIX (%)", style=label_style),
                            html.Div(_format_number(MIX, 1), style=value_style),
                            html.Div("ADS vs All Outlets", style=sub_style),
                        ]
                    ),
                    style=card_style,
                ),
...
# -------------------------------------------------
# LAYOUT FACTORY FOR main_app.py
# -------------------------------------------------
def get_layout():
    """Return the layout for the DSR page (used by main_app.py)."""
    global summary_df_init, mis_df_init, weekly_mis_init
        ...
        children=[
            dbc.Container(
                ...
                children=[
                            ...
                            dbc.Col(
                                [
                                    ...
                                    dcc.Dropdown(
                                        ...
                                        multi=False,
                                        placeholder="Running Month (auto)",
                                        style={"fontSize": "13px"},
                                    ),
                                ],
                                md=3,
                    ...
                    dbc.Row(
                        [
                            dbc.Col(
                                [
                                    html.Label(
                                        "Summary search (matching letters on Outlet Name)"
                                    ),
                                    dcc.Input(
                                        id="search_text",
                                        type="text",
                    ...
                    # KPI cards row (month-wise ADS / AOV / ADT / MIX)
                    html.Div(id="dsr-kpi-row"),
                            ...
                            dcc.Tab(
                                ...
                                children=[
                                    ...
                                    dash_table.DataTable(
                                        id="mis_table",
                                        columns=[
                                            {"name": c, "id": c} for c in MIS_COLUMNS
                                        ]
                                        if MIS_COLUMNS
                                        else [],
                                    ...
                                    dbc.Row(
                                        [
                                            ...
                                            dbc.Col(
                                                [
                                                    html.H5(
                                                        "Monthly ADS Comparison (MTD vs LMTD)"
                                                    ),
                                                    dcc.Graph(
...
# -------------------------------------------------
# CALLBACK REGISTRATION (for main_app.py)
# -------------------------------------------------
def register_callbacks(app):
    """Attach DSR callbacks to the main Dash app."""
    ...
    def update_city_outlet_options(region_vals, city_vals, n_clicks):
        ...
        # (small fix: comprehension was slightly wrong above – correct below)
        outlet_options_local = [{"label": o, "value": o} for o in outlets]
    ...
    def update_week_options(month_val, n_clicks):
        """Week dropdown options mapped to chosen Month."""
        global sales_df
        df = sales_df
        if df.empty:
            return []
    ...
    def update_dashboard(
        ...
        if df.empty:
            ...
            kpi_children = html.Div(
                "No data available", style={"padding": "4px", "fontSize": "13px"}
            )
            return kpi_children, empty_data, empty_data, empty_data, fig, fig, fig

        # Build KPI cards first (month-wise, based on Region/City/Outlet/Month filters)
        kpi_children = build_kpi_cards(region_vals, city_vals, outlet_vals, month_val)

        # Apply filters for tables/charts
        if region_vals:
            df = df[df["Region"].isin(region_vals)]
        ...
        if not weekly.empty:
            ...
            if city_vals and "City" in df.columns:
                if isinstance(city_vals, list):
                    outlets_city = df[df["City"].isin(city_vals)][
                        "Outlet Name"
                    ].unique()
                else:
        ...
        # CHARTS
        # ADS trend (Overall per Outlet per Date)
        if df.empty:
            ...
            if source_vals:
                real_src = [
                    s for s in source_vals if s in ["Dine In", "Delivery", "App", "Other"]
                ]
                if real_src:
